...
export default function TestModule({ questions, lessonId, topicNumber, onComplete, bestScore }: TestModuleProps) {
    ...
    return (
      <div className="flex flex-col items-center py-8 text-center">
        <div className={cn(
          ...
          passed
            ? 'bg-emerald-600/10 border-2 border-emerald-600/30 text-emerald-600'
        ...
        <div className="grid grid-cols-3 gap-3 w-full max-w-xs mb-8">
          {[
            { label: 'Ball', value: `${score}/${questions.length}`, color: 'text-primary' },
            { label: 'Baho', value: grade, color: gradeColor(grade) },
            { label: 'Natija', value: scoreLabel(pct), color: passed ? 'text-emerald-600' : 'text-red-400' },
          ].map((s) => (
        ...
        <div className="w-full space-y-2 text-left mb-8 max-w-lg">
          {questions.map((q, i) => {
            ...
            return (
              <div key={q.id} className={cn('flex items-start gap-3 p-3 rounded-xl border', correct ? 'border-emerald-600/20 bg-emerald-600/5' : 'border-red-500/20 bg-red-500/5')}>
                {correct
  ...
  // ── Quiz ──
  const timerColor = timeLeft < 60 ? 'text-red-400' : timeLeft < 120 ? 'text-accent-copper-light' : 'text-text-secondary'
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2.5">
        {currentQ.options.map((opt, i) => {
          let style = 'border-border bg-surface-2 text-text-secondary hover:border-primary/40 hover:bg-primary/5'
          if (selected === i && !revealed) style = 'border-primary/60 bg-primary/10 text-primary'
          if (revealed) {
            if (i === currentQ.correctIndex) style = 'border-emerald-600/60 bg-emerald-600/10 text-emerald-600'
